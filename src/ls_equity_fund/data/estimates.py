"""Daily analyst-estimates snapshot (DATA-09).

Fans out across the active universe, calls ``provider.get_estimates(ticker,
today)`` per name, and persists one row per ticker per day in
``analyst_estimates`` (PK ``(ticker, snapshot_date)``). Re-running the same
day is idempotent (INSERT OR IGNORE).

Per-ticker failures are log+continue (same pattern as Plan 01-04 prices
ingest). The 30/60/90-day estimate-revisions factor (Phase 2) reconstructs
...
def refresh_estimates(
    ...
    *,
    ...
    """Refresh today's analyst-estimates snapshot across the active universe.
    ...
    Args:
        ...
        today: optional date override (default: ``date.today()``).
    ...
    try:
                ...
                try:
                    ...
                    conn.execute(
                        """INSERT OR IGNORE INTO analyst_estimates
                           (ticker, snapshot_date, eps_fy1, eps_fy2,
                            rev_fy1, rev_fy2, target_price, n_analysts)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                        (
                            ticker,
                            today_str,
                            snapshot.get("eps_fy1"),
                            snapshot.get("eps_fy2"),
                            snapshot.get("rev_fy1"),
                            snapshot.get("rev_fy2"),
                            snapshot.get("target_price"),
                            snapshot.get("n_analysts"),
                        ),
                    )
                    rows_written += 1
        ...
        log.info("refresh_estimates_complete", **result)
        return result
    finally:
        if owns_conn:
            conn.close()
...
def _persist_state(
    ...
    """Upsert a refresh_state row for (yfinance, estimates, ticker)."""
    conn.execute(
